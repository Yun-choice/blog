...
2. 날짜 변수를 열 선반에 위치한 후, 연속형 '월' 필드로 바꿔줍니다.
	...
	![[Utilities/attatchments/Pasted image 20251224005640.png|400]]

3. 마크 상자에 앞서 생성한 파생변수를 아래와 같이 추가합니다.

	![[Utilities/attatchments/Pasted image 20251224005952.png|150]]

4. 마크 → 레이블에서 '마크 레이블 표시'를 체크합니다.
5. 제목을 더블클릭하여 제목 편집으로 진입한 후, 아래와 같이 수정합니다

	![[Utilities/attatchments/Pasted image 20251224010236.png|500]]

6. 필터 상자에 날짜 변수를 할당하고, 가장 최신 연도로 설정합니다.

- 결과

	![[Utilities/attatchments/Pasted image 20251224010602.png|600]]
...
1. 열 선반의 빈 공간을 더블클릭하여 더미 변수 두 개를 만들어줍니다. 저는 `AVG(0)`이라 입력하여 만들었습니다.
	...
	![[Utilities/attatchments/Pasted image 20251224011722.png|400]]
...
4. '이중축' 기능을 사용하여 두 그래프를 하나로 합쳐줍니다.
	...
	![[Utilities/attatchments/Pasted image 20251226091749.png|400]]

5. 첫 번째 '집계(AVG(0))' 탭을 아래와 같이 설정해줍니다. '색상'에는 표현하고 싶은 범주형 변수를 할당합니다. 여기까진 일반적인 파이차트와 다르지 않습니다.

	![[Utilities/attatchments/Pasted image 20251226092043.png|200]]

6. 두 번째 '집계(AVG(0))' 탭에 매출 합계 정보를 표시합니다. 레이블을 아래와 같이 구성합니다. 기타 서식도 취향껏 설정합니다.

	![[Utilities/attatchments/Pasted image 20251226092806.png|550]]

- **결과**

	![[Utilities/attatchments/Pasted image 20251226093927.png|800]]
	![[Utilities/attatchments/Pasted image 20251226094235.png|600]]
...
3. 마크 선반에 미리 만들어둔 파생변수를 아래와 같이 위치하고, 레이블 편집에서 서식을 아래와 같이 수정합니다.
	...
	![[Utilities/attatchments/Pasted image 20251226143100.png|500]]
	`<집계(Negative % Diff)> <집계(Positive % Diff)> <집계(No Sales)> <집계(% Difference)>`

4. 집계(% Difference)를 우클릭, '숫자 서식에서' '백분율'로 바꿔줍니다.

	![[Utilities/attatchments/Pasted image 20251226143746.png|200]]

- 중간 결과

	![[Utilities/attatchments/Pasted image 20251226143926.png|550]]
...
1. 'YTD Revenue'를 열 선반에 하나 더 추가하고 '이중 축'을 걸어줍니다.
	...
	![[Utilities/attatchments/Pasted image 20251226144405.png|400]]

2. 위에 새로 생겨난 축을 우클릭, 어긋난 축을 하나로 동기화해줍니다.

	![[Utilities/attatchments/Pasted image 20251226145143.png|400]]
...
4. 레이블 모양을 왼쪽 정렬로 설정합니다. (레이블 모양은 Tableau Desktop Public Edition에서만 수정 가능한 것 같습니다.)
	...
	![[Utilities/attatchments/Pasted image 20251226150346.png|350]]

5. 기타 글꼴 및 서식을 취향껏 바꿔줍니다.

- **결과**

	![[Utilities/attatchments/Pasted image 20251226150618.png|600]]
...
2. 행 선반에 판매량 변수를 두 번 올리고, '이중축'을 설정합니다. (이번엔 Qty라는 판매량 order_quantity로 그래프를 그려봤습니다.)
	...
	![[Utilities/attatchments/Pasted image 20251226153541.png|400]]

3. 마크 선반에서 '합계(Qtry)(2)'의 그래프 종류를 '원' 그래프로 바꿔줍니다.

	![[Utilities/attatchments/Pasted image 20251226153728.png|200]]

4. 판매 채널이 들어가 있는 범주형 변수를 '합계(Qty)'와 '합계(Qty)(2)' 모두에 색상 요소로 추가합니다.

	![[Utilities/attatchments/Pasted image 20251226154225.png|200]]

5. 행 선반에서 두 '합계(Qty)' 모두에게 '테이블 계산을' 부여합니다.

	![[Utilities/attatchments/Pasted image 20251226154352.png|400]]

	- 계산 유형: 순위
	- 계산 대상: 특정 차원 -> 'Site_grouped'

	![[Utilities/attatchments/Pasted image 20251226154558.png|300]]
6. 행 선반의 '합계(Qty)'를 Ctrl을 누른채 드래그하여 '합계(Qty)(2)'의 레이블로 복사 및 추가합니다. 또한, 레이블 모양에서 정렬을 가운데로 맞춥니다.

	![[Utilities/attatchments/Pasted image 20251226155012.png|200]]

7. 마크 선반의 '합계(Qty)'에 'Site_grouped' 변수를 레이블로 추가하고, 레이블 지정할 마크를 '가장 최근'으로 설정합니다.

	![[Utilities/attatchments/Pasted image 20251226155248.png|300]]

8. 기타 글꼴 및 서식을 취향껏 설정해줍니다.

- **결과**

	![[Utilities/attatchments/Pasted image 20251226155446.png|600]]
...
<<<<<<< HEAD
1. 만든 그래프 중 아무 곳에서, '필터' 상자에 판매 채널 변수 'Site_grouped'와 지역 변수 'Region 1Dep'을 추가합니다. ([[Projects/신발회사 Sale 데이터로 캡스톤 디자인 진행하기/1. Design Thinking으로 과제 정의하고 Sales 데이터 분석하기#3) region_1depth, lon/lat\|1편에서 만든 파생변수]]가 여기서 빛을 발하네요)
=======
1. 만든 그래프 중 아무 곳에서, '필터' 상자에 판매 채널 변수 'Site_grouped'와 지역 변수 'Region 1Dep'을 추가합니다. ([[Projects/신발회사 Sale 데이터로 캡스톤 디자인 진행하기/1. Design Thinking으로 과제 정의하고 Sales 데이터 분석하기#3) region_1depth, lon/lat region1depth\|1편에서 만든 파생변수]]가 여기서 빛을 발하네요)
>>>>>>> origin/v4
2. 각각 우클릭 -> '컨텍스트에 추가'를 설정합니다. 이로서 모든 워크시트에 해당 필터들이 관여하게 됩니다.
3. [[Projects/신발회사 Sale 데이터로 캡스톤 디자인 진행하기/3. Tableau로 Sales KPI 대시보드 개발하기#4) 월별 판매 채널 순위 Bump chart]]에는 필터 상자에 날짜 변수를 추가하고, 범주형 연도로 설정합니다.

- **결과**


	![[Utilities/attatchments/Pasted image 20251226160914.png|250]]
...
1. 우선 대시보드의 크기를 설정해줍니다. 저는 PowerPoint 크기로 설정해주었습니다. (사실 '자동'으로 했다가 나중에 변경하는 것이 편합니다.)
	...
	![[Utilities/attatchments/Pasted image 20251226161343.png|250]]

2. 바둑판식 대시보드에 빈페이지를 추가하고, 가로 컨테이너를 위에 추가합니다.

	![[Utilities/attatchments/Pasted image 20251229155153.png|600]]

3. 만들어둔 시트를 하나씩 추가합니다.

	![[Utilities/attatchments/Pasted image 20251229155652.png|600]]
...
4. Region 필터를 추가합니다. ([[Projects/신발회사 Sale 데이터로 캡스톤 디자인 진행하기/3. Tableau로 Sales KPI 대시보드 개발하기#1) 필터링\|필터링]] 단계에서 설정한 '컨텍스트' 필터가 반드시 선행되어야 합니다.)
	...
	![[Utilities/attatchments/Pasted image 20251229160700.png|600]]

5. Site Rank 시트 설정에서 '필터' → 날짜 변수의 '연도'를 체크합니다.

	![[Utilities/attatchments/Pasted image 20251229161523.png|500]]

6. 대시보드 상단에 '빈 페이지'와 '텍스트'를 추가하여 대시보드 제목과 빈 공간을 만들어줍니다.

	![[Utilities/attatchments/Pasted image 20251229162059.png|600]]

7. 필터와 범례를 '부동'을 체크하여 알맞은 위치에 재배치하고, 범례 컨테이너를 없애줍니다.

	![[Utilities/attatchments/Pasted image 20251229162242.png|200]]

	![[Utilities/attatchments/Pasted image 20251229162448.png|600]]
...
- **최종 결과** [Link](https://public.tableau.com/views/ShoesSalesKPIDashboard/ShoesSalesKPIdashboard?:language=ko-KR&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link)
	...
	![[Utilities/attatchments/Pasted image 20251224012328.png|800x]]
